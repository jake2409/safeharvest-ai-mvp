...
// Regiões agrícolas usadas no MVP.
// Depois você pode salvar isso em banco ou deixar o usuário cadastrar regiões.
const regions = [
...
// Open-Meteo não entrega NDVI nem turbidez.
// Para o MVP, esses indicadores são estimados/simulados a partir dos dados climáticos.
// Na versão real, NDVI viria de Sentinel-2/Sentinel Hub e turbidez de dados de água/sensoriamento remoto.
...
async function buildRegionRisk(region) {
  ...
  const rain7d = Number(
    hourlyPrecipitation.reduce((total, value) => total + Number(value || 0), 0).toFixed(1)
  );
...
app.get('/api/regions', async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: 'Não foi possível buscar os dados climáticos.',
      error: error.message
    });
  }
});
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    const summary = result.reduce((acc, region) => {
      acc.total += 1;
      acc[region.riskLevel.toLowerCase()] += 1;
      acc.averageRisk += region.riskScore;
      return acc;
    }, { total: 0, alto: 0, médio: 0, baixo: 0, averageRisk: 0 });
    ...
    res.json({
      ...
      criticalRegions: result
        .filter(region => region.riskLevel === 'Alto')
        .map(region => `${region.city} - ${region.state}`)
    });
  } catch (error) {
    res.status(500).json({
...
app.listen(port, () => {
  console.log(`SafeHarvest AI rodando em http://localhost:${port}`);
});
